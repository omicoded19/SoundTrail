...
function getSearchQueries(query: string) {
  ...
  const cleanedQuery = trimmedQuery
    .replace(
      /\b(?:official|video|audio|lyrics?|song|full|version|hd|4k)\b/gi,
      ' ',
    )
    .replace(/[()[\]{}"'`]+/g, ' ')
    .replace(/\s+/g, ' ')
  ...
  return [
    ...new Set(
      [trimmedQuery, cleanedQuery].filter(
...
function sortArtistsForQuery(
  ...
  return [...artists].sort((firstArtist, secondArtist) => {
    const priorityDifference =
      getPriority(firstArtist.name) -
      getPriority(secondArtist.name)
...
function getTrackSearchPriority(
  track: Track,
  query: string,
) {
  const normalizedQuery = normalizeSearchText(query)
  const normalizedTitle = normalizeSearchText(track.title)
  const normalizedArtist = normalizeSearchText(
    track.artistName,
  )
  const normalizedAlbum = normalizeSearchText(
    track.albumTitle,
  )
  ...
  if (normalizedAlbum === normalizedQuery) {
    return 5
  }
...
function sortTracksForQuery(
  tracks: Track[],
  query: string,
) {
  return [...tracks].sort((firstTrack, secondTrack) => {
    const priorityDifference =
      getTrackSearchPriority(firstTrack, query) -
      getTrackSearchPriority(secondTrack, query)
...
function trackBelongsToArtist(
  ...
  const normalizedTrackArtist =
    normalizeSearchText(trackArtistName)

  const normalizedArtist =
    normalizeSearchText(artistName)

  if (!normalizedTrackArtist || !normalizedArtist) {
    return false
  }

  return (
    normalizedTrackArtist === normalizedArtist ||
...
function getPrimaryArtistForQuery(
  artists: SoundTrailArtist[],
  tracks: Track[],
  query: string,
) {
  ...
  const likelyArtist = artists.find((artist) => {
    const normalizedArtistName =
      normalizeSearchText(artist.name)

    const nameLooksRelevant =
      normalizedArtistName.startsWith(normalizedQuery) ||
      normalizedQuery.startsWith(normalizedArtistName)

    if (!nameLooksRelevant) {
      return false
    }

    const matchingTrackCount = tracks.filter((track) =>
    ...
    const requiredMatches = Math.min(
      ...
      Math.max(
        1,
        Math.ceil(tracks.length * 0.35),
      ),
    )

    return matchingTrackCount >= requiredMatches
  })
...
export function DiscoverPage() {
  ...
  useEffect(() => {
    ...
    async function loadQuickPicks() {
      ...
      try {
        ...
        const uniqueResults = getUniqueTracks(
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      try {
        const searchQueries =
          getSearchQueries(trimmedQuery)

        const [trackResults, artistResult] =
          await Promise.all([
            Promise.allSettled(
        ...
        const loadedTracks = trackResults.flatMap(
          (result) =>
            result.status === 'fulfilled'
              ? result.value
              : [],
        ...
        const loadedArtists =
          artistResult[0]?.status === 'fulfilled'
            ? artistResult[0].value
            : []

        const sortedTracks = sortTracksForQuery(
          getUniqueTracks(loadedTracks),
          trimmedQuery,
        ).slice(0, 24)

        const sortedArtists = sortArtistsForQuery(
          loadedArtists,
          trimmedQuery,
        ).slice(0, 6)
        ...
        const allTrackRequestsFailed =
          trackResults.every(
            (result) => result.status === 'rejected',
          )

        const artistRequestFailed =
          artistResult[0]?.status === 'rejected'

        if (
          allTrackRequestsFailed &&
          artistRequestFailed
        ) {
          const firstFailure = trackResults.find(
            (result) => result.status === 'rejected',
          )
  ...
  function handlePlayTrack(
    ...
    const playableQueue = sourceQueue.filter(
      (queueTrack) =>
      ...
      <section className="mx-auto max-w-7xl">
        <header>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-white/55 md:text-base">
            Search by song title, artist, album or
            genre and play available catalogue
            previews.
          </p>
        </header>
          ...
          <div className="mt-10 space-y-14">
            <section>
              <SectionHeading
                ...
                description="Choose a category to search its real music catalogue."
              />
...
function SearchResults({
  ...
  const primaryArtist = getPrimaryArtistForQuery(
    artists,
    tracks,
    query,
  )
  ...
  const otherArtists = artists.filter(
    (artist) => artist.id !== primaryArtist?.id,
  )

  const hasResults =
    tracks.length > 0 || primaryArtist !== null
      ...
      {!isLoading && !error && !hasResults && (
        <div className="flex min-h-72 flex-col items-center justify-center rounded-3xl border border-dashed border-white/10 bg-white/[0.03] px-6 text-center backdrop-blur-xl">
          ...
          <p className="mt-2 max-w-md text-sm text-white/50">
            Try a song title, artist, album or a shorter
            phrase.
          </p>
        </div>
      )}
        ...
        <div className="space-y-14">
          <section>
            <SectionHeading
              ...
              description={
                primaryArtist
                  ? `${primaryArtist.name} and matching catalogue songs`
                  : `${tracks.length} matches across titles, artists and albums`
              }
            />
            ...
            <div
              className={`mt-6 grid items-start gap-6 ${
                primaryArtist
                  ? 'lg:grid-cols-[minmax(300px,0.8fr)_minmax(0,1.7fr)]'
                  : 'grid-cols-1'
              }`}
            >
              {primaryArtist && (
              ...
              <div
                className={`min-w-0 rounded-3xl border border-white/10 bg-white/[0.035] p-4 shadow-xl shadow-black/20 backdrop-blur-2xl md:p-5 ${
                  primaryArtist
                    ? 'lg:max-h-[620px] lg:overflow-hidden'
                    : ''
                }`}
              >
                ...
                {tracks.length > 0 ? (
                  <div
                    className={
                      primaryArtist
                        ? 'mt-5 space-y-3 lg:max-h-[510px] lg:overflow-y-auto lg:pr-2'
                        : 'mt-5 grid gap-3 md:grid-cols-2 2xl:grid-cols-3'
                    }
                  >
                    {tracks.map((track) => (
                      <SearchTrackCard
                  ...
                  <div className="mt-5 flex min-h-64 flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 bg-black/10 px-6 text-center">
                    ...
                    <p className="mt-2 text-sm text-white/45">
                      An artist match was found, but there
                      is no playable preview for this
                      search.
                    </p>
                  </div>
                )}
          ...
          {otherArtists.length > 0 && (
            <section>
              <SectionHeading
                ...
                description="Artist profiles related to the words you searched."
              />
  ...
  return (
    ...
    >
      <button
        ...
        disabled={!canPlay}
        className="relative size-14 shrink-0 overflow-hidden rounded-xl bg-white/10 disabled:cursor-not-allowed disabled:opacity-50"
        aria-label={
          canPlay
            ? activePlaying
              ? `Pause ${track.title}`
      ...
      <div className="flex shrink-0 flex-col items-end gap-1">
        ...
        <button
          type="button"
          onClick={() => void onToggleLike(track)}
          aria-label={
            liked
              ? `Unlike ${track.title}`
              : `Like ${track.title}`
